...
class Theatre:
    ...
    def DisplayMovies(self):
        print('List Of Movies In Our Record :- \n')
    ...
    def LendMovie(self, movieid, moviename, user):
        ...
        if check!=None:
            ...
            print('\nDATABASE UPDATED SUCCESSFULLY. YOU CAN LEND YOUR MOVIE NOW.\n ')
            cursor.execute("SELECT _id, movie_name, user_name, trans_time FROM entries;")
            for row in cursor:
                print('{} | {} | {} | {}'.format(row[0], row[1], row[2],row[3]))
            connection.commit()
    ...
    def ReturnMovie(self, movieid):
        ...
        for val in cursor:
            person = val[0]
        print("MOVIE SUCCESSFULLY RETURNED by user : {}.".format(person))
...
if __name__ == '__main__':
    movieBase = Theatre("MakeInBharat")
    ...
    while(True):
        ...
        if user_choice == 1:
            ...
            print("\nMovies You might also like to watch out : \n")
